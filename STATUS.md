...
- **2026-07-12 (audit: read-only — Bug #4 / Bug #7+3 foregrounding)** — After the port-swap work landed
  cleanly (lint 0 / vitest 89/89 / build 0), and while waiting for Team A's P1C-server-side build to finish,
  took two read-only investigations to size what Bug work in Repo B does NOT depend on Team A's gated authz
  resolver (the rest of P1C does).
  **(2C) Bug #4 Dexie-only-override audit (handoff 002 §4):** Repo B's `src/features/Auth.tsx` L135
  `db.users.update(uname, { role: newRole })` + L425/L443 `db.users.update(username, { customPermissions: ... as any })`
  write per-user role + per-feature permission overrides DIRECTLY to Dexie, and `src/App.tsx` L66
  ("Safeguard page permission redirects") reads `currentUser` from `localStorage: puv_logged_user` + Dexie.
  **The leak signature IS present in Repo B.** BUT: grep of `src/features/Auth.tsx` for `supabase`/`from(`/`rpc(`
  returned zero hits — the entire Auth UI's user-management API (login, signup, approve, set-role,
  set-custom-permissions) calls only Dexie, not Supabase. No server is queried for the users/auth domain.
  Cross-reading AGENTS.md §3 Engineering Loop and Team A's STATUS row (their P1A/P1B includes "REAL-CLOUD E2E:
  owner login → tester signed up via the UI → email-confirmed → pending & blind → owner approved in the queue
  → membership landed on cloud"), the two "Phase 1 done" lines mean materially different things across the two
  repos: Team A has the online seam (mock→Dexie | online→PostgREST/RPC | offline→outbox) WIRED for the users
  domain; Repo B does NOT have it yet — Repo B's auth is mock-mode prototype only. **Conclusion: the Bug #4
  Dexie leak is technically present in Repo B but MOOT** — there is no server authority to bypass while the
  app doesn't call Supabase auth/`has_permission`/`public.users` for this domain. The real work needed in
  Repo B isn't "harden the Dexie leak" — it's "build the online seam so the app actually talks to the server,"
  at which point the Dexie values become a client cache of server-truth and the leak becomes a live concern
  that the eventual P1C port (when Team A finishes) solves together with the resolver evolution. The P1C
  port, sized correctly, IS the online-seam build + the resolver hardening, not just "patch Dexie writes."
  **(1A) 14-guard SQL sweep attempt via Supabase Studio browser editor** — after the `docker exec`
  container-name lookup quirk blocked the standard sweep path (recorded in the port-swap entry above),
  attempted the sweep through Studio's in-browser Monaco SQL editor at `http://127.0.0.1:54523`. Proved the
  round-trip works end-to-end: `window.monaco.editor.getModels()[0].setValue(...)` + Run button returns
  results. But the JS-evaluator string transport truncated multi-KB SQL at ~1.5KB from a 2.7KB source — three
  documented variants (template-literal parses to SyntaxError; raw atob b64 rejects with "Invalid or unexpected
  token"; clean b64 truncated to 1573 chars from 2689). No `/api/*` SQL POST endpoint exists (404s on every
  likely path). Per SESSION_PROMPT E7 (three failed fix attempts on one path = HARD STOP) the 14-guard sweep
  via Studio is NOT viable from this terminal. The 25 migrations applied cleanly via `supabase db reset` to the
  same Postgres the guards target, so the underlying DB state is clean; only the guard-battery re-verification
  is blocked. CI re-verifies all 14 guards on the next push (CI inherits new ports automatically). Two stray
  `supabase/snippets/Untitled query *.sql` files Studio auto-saved during the exploration were removed; added
  `supabase/snippets/` to `.gitignore` to prevent future Studio uses from polluting the repo working tree.

- **2026-07-12 (app-bugs #7 #3: handoff 002 Phase-1 non-gated UI bugs fixed in Repo B)** — The two P1C bugs
  that are PURE app-side (no DB, no auth-resolver, no `has_permission` evolution) and independent of Team A's
  gated P1C server work — Bug #7 (top-bar Sign-Out, handoff 002 §7) and Bug #3 (AwaitingApproval auto-poll +
  on focus, handoff 002 §3) — shipped locally as one logical change in three files.
  **Bug #7 (Sign-Out removed from top bar, logout is Settings → Session only):** deleted the desktop top-bar
  Sign-Out `<button onClick={handleLogout} ...>` block (`src/App.tsx` L285-292) and replaced with a comment
  noting the lands-the-Settings-panel-only rule. The `handleLogout` fn at `src/App.tsx` L79 RETAINED (calls
  `localStorage.removeItem('puv_logged_user')` + `setCurrentUser(null)`) because `Settings.tsx` still uses it
  via the `onLogout` prop pass at `src/App.tsx` L352. The mobile hamburger-drawer Sign Out (L562-570) is
  intentionally KEPT (handoff 002 §7 specifies "top bar" only; mobile has no top-bar affordance and the
  drawer's Settings Hub link sits right above it). No dead imports introduced (`LogOut` icon still imported
  because the mobile drawer button still uses it).
  **Bug #3 (AwaitingApproval auto-poll):** extended React imports on `src/features/Auth.tsx` L1 with `useRef`
  and lucide imports with `RefreshCw`. Added a `useEffect` (runs only when `pendingApproval === true` AND
  `username` is non-empty) that: (1) on mount, calls `checkApproval` once so a race-approve-during-signup is
  caught; (2) sets a 15-second interval that re-reads `db.users.get(username.trim().toLowerCase()).approved`
  from Dexie; (3) registers a `window.focus` listener so focusing the tab immediately re-checks (matches
  Team A's STATUS L253 "auto-polls refresh() every 15s + on focus" pattern adapted to mock-mode — instead of an
  RPC `refresh()` call we read Dexie, the workflow authority per AGENTS.md §3); (4) when `approved === true`,
  runs the SAME post-approval sequence the manual flows use (`setPendingApproval(false) → setCurrentUser(u)
  → localStorage.setItem('puv_logged_user', u.username) → onRefresh()` — mirrors Auth.tsx L53-55 + L87-89 +
  L162-166); (5) cleanup clears interval + removes focus listener. Added a new visual hint inside the
  AwaitingApproval card: `<RefreshCw animate-spin style={{animationDuration: '3s'}} /> Checking approval
  status every 15s and on window focus…` so the user sees the poll is active. Added a code comment explaining
  the mock-mode source-of-truth and noting the eventual replacement with `has_permission`/`auth.refresh()`
  RPC when the online seam lands. Negative path is guarded: a `false` reading of `approved` is a no-op
  (interval keeps polling) — no false-success path per AGENTS.md bug-pattern #4.
  **Verified first-hand this session (at the post-bug-fix tree, before commit):** `npm run lint` (tsc --noEmit)
  exit 0; `npm run test` (vitest run) **18 files / 89 tests / 0 fail** (4.52s after Bug #7, 3.89s after Bug #3);
  `npm run build` (vite build) ✓ built in 9.32s exit 0. **NOT verified locally this session (honest):** the
  browser mock-mode E2E for Bug #7's "no Sign Out in top bar" + Bug #3's "auto-redirect on approval" claims
  — `vite dev` server at http://localhost:3000 started cleanly + the login page rendered, but
  `browser_type` on the controlled Email/Password inputs does not trigger React's onChange handler
  (synthetic typing writes the DOM value but doesn't update the React `username`/`password` state, so the
  form submit silently lands on the empty-field early-return at `Auth.tsx` L45-48 → no error visible, no
  login advance). localStorage mutation to seed a logged-in session was blocked by the browser console's
  sensitive-primitive guard (web-storage primitives are protected even on local mock-mode). Per A→B→C + E7,
  four attempts to drive via keyboard/Enter/Tab/focus produced zero advance — HARD STOP. The Bug #7 and
  Bug #3 patches are small + self-evident by code inspection (patches diff at `src/App.tsx` L285-292 →
  comment-only, and `src/features/Auth.tsx` 30 lines added around L138), and lint/test/build all pass; only
  the in-browser behavior-of-the-DOM E2E walk is skipped (CI runs the unit suite but does not run browser
  E2E either — both Auto-Bug-7+3 and the eventual Team-A port should pick up browser E2E in a future session
  once the form-event-generation tooling friction is addressed). **Files touched (3):** `src/App.tsx`,
  `src/features/Auth.tsx`, `.gitignore` (added `supabase/snippets/` to prevent Studio private-query
  auto-saves from polluting the repo — discovered during the 1A Studio attempt). Cloud project
  `jabjyvdkadcbfocaerno` untouched. Repo A untouched (boundary rule). P1C gated work (per-user overrides
  server enforcement, has_permission evolution, rank-based revoke, signup-trigger queue fix) NOT started —
  still waiting for Team A to finish their P1C build + the owner's per-port GO before any port work begins.