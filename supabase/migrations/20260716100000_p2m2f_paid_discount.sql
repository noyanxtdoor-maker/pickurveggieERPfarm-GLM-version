-- ════════════════════════════════════════════════════════════════════════════
-- 20260716100000_p2m2f_paid_discount.sql
-- Owner directive (2026-07-16, Tier 2.4 batch): "add an option of 10% for both the pre-order
-- and the direct cash clearance tab" -- the 10% Farm Discount toggle should be available on the
-- PAID (walk-in cash) tab too, not just pre-order. The server must accept p_discount_rate=0.10
-- when p_sale_kind='paid'.
--
-- What changes vs the live (P2-M2E) function:
--   - The constraint `p_sale_kind = 'paid' and (p_discount_rate <> 0 or coalesce(p_delivery_fee, 0) <> 0)
--     then raise 'discount/delivery apply to pre-orders only'` is SPLIT. Only the delivery-fee half
--     remains. A paid sale MAY now carry p_discount_rate in {0, 0.10} (server-constrained -- no
--     client can send 0.05 or 0.25). Delivery is still pre-order-only (the user did not ask for
--     paid delivery).
-- Everything else is byte-for-byte the P2-M2E function (prototype parity, farm price = retail x
-- 0.90 per weighed line; bulk wholesale flat-price lines; paid|preorder(credit->AR); atomic +
-- balanced GL; idempotent; audited; COGS pair when v_cogs>0). Same signature, same comment bucket.
--
-- Authoritative spec: this migration IS the spec for the paid-discount relaxation. Everything else
-- inherits P2-M2E's spec (Phase_2_M2E_Prototype_Parity_Spec.md + src/lib/money.ts DISCOUNT=0.10).
-- ════════════════════════════════════════════════════════════════════════════

-- drop the LIVE (P2-M2E) 8-arg signature -- the 20260702180000 migration created this shape,
-- superseding the original 4-arg form from 20260628130000 and the intermediate form from
-- 20260702090000. Same 8-arg signature is recreated below by drop+recreate (immutable-migration
-- rule per AGENTS.md section 2: evolve the function via drop+recreate in a NEW migration file).
...
begin
  ...
  -- P2-M2F (Tier 2.4, 2026-07-16): the paid-discount ban is lifted. A paid sale MAY carry
  -- p_discount_rate in {0, 0.10}. Delivery fee remains pre-order-only (paid delivery not asked
  -- for; the constraint below retains the delivery-fee half of the old combined guard).
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    if v_line ? 'bulk_price' then
      -- Bulk wholesale (mock "Skip Weigh"): negotiated flat price; no weight -> no movement, no COGS (spec section 3).
      ...
      -- Weighed produce: SERVER computes the farm price (retail x 0.90) -- the mock's DISCOUNT=0.10, B2 rounding.
...
comment on function public.pos_record_sale(uuid, jsonb, numeric, text, text, numeric, numeric, text) is
  'P2-M2F (Tier 2.4, 2026-07-16): same as P2-M2E (farm price = retail x 0.90, bulk lines, paid|preorder, balanced GL, idempotent, audited) but the 10% Farm Discount is now allowed on BOTH paid and pre-order paths. Delivery fee still pre-order-only. Drop+recreate of the 20260702180000 function -- same signature, only the guard body changed.';

revoke all on function public.pos_record_sale(uuid, jsonb, numeric, text, text, numeric, numeric, text) from public, anon;
